...
class TestClustering(unittest.TestCase):
    def test_cluster_mnist(self):
        ...
        cache_manager.set_cluster_user_cnt(list(range(100)))
        cache_manager.eval()
    ...
    def test_cluster_cifar10(self):
        np.random.seed(0)
        n_users, n_cluster, tau = 100, 5, 0.1
        conf_file = "./config/fedtts-conf.yaml"
        cache_manager = RoundCacheManager()
        with open(conf_file, "r") as conf_yaml:
            cluster_config = yaml.safe_load(conf_yaml)
        dataset = torchvision.datasets.CIFAR10(root="./data/cifar", download=True)
        user_datapoints, idx_labels = cluster_dataset(
            dataset, n_users, n_cluster, tau, cluster_config["CIFAR"], cache_manager
        )
        cache_manager.eval()
        print(len(user_datapoints))
        overlapping_cnt = 0
